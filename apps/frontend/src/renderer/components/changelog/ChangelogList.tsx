...
export function ChangelogList({
  ...
  const getSummaryLabel = () => {
    switch (sourceMode) {
      case 'tasks':
        return t('changelog:summary.task');
      case 'git-history':
      case 'branch-diff':
        return t('changelog:summary.commit');
      default:
        return t('changelog:summary.item');
    }
  };
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('changelog:selection.countStatus', {
              ...
              <div className="flex gap-1">
                ...
                >
                  {t('changelog:selection.selectAll')}
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
            ...
            {doneTasks.length === 0 ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('changelog:empty.noCompletedTasks')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {t('changelog:empty.noTasksDescription')}
                  </p>
                </div>
              </div>
            ) : (
      ...
      {(sourceMode === 'git-history' || sourceMode === 'branch-diff') && (
        <>
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('changelog:commits.found', {
                  count: previewCommits.length,
                  plural: previewCommits.length !== 1 ? 's' : ''
                })}
              </span>
              {isLoadingCommits && (
            ...
            {isLoadingCommits ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <p className="mt-4 text-sm text-muted-foreground">{t('changelog:commits.loading')}</p>
                </div>
              </div>
              ...
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('changelog:empty.noCommitsFound')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('changelog:empty.noCommitsGitHistory')
                      : t('changelog:empty.noCommitsBranchDiff')}
                  </p>
                </div>
              </div>
            ) : (
