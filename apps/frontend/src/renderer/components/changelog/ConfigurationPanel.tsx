      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          <Button variant="ghost" size="sm" onClick={onBack} className="-ml-2">
            ...
            {t('config.actions.backToSelection')}
          </Button>
          <div className="rounded-lg bg-muted/50 p-3">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              {t('config.summary.including', { count: summaryInfo.count, label: summaryInfo.label })}
            </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{t('config.labels.releaseInfo')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="version">{t('config.labels.version')}</Label>
              <Input
                id="version"
                value={version}
                onChange={(e) => onVersionChange(e.target.value)}
                placeholder={t('config.placeholders.version')}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="date">{t('config.labels.date')}</Label>
              <Input
                id="date"
                type="date"
                value={date}
            ...
            {(existingChangelog?.lastVersion || versionBumpDescription) && (
              <div className="text-xs text-muted-foreground space-y-1">
                {existingChangelog?.lastVersion && (
                  <p>{t('config.labels.previousVersion', { version: existingChangelog.lastVersion })}</p>
                )}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{t('config.labels.outputStyle')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>{t('config.labels.format')}</Label>
              <Select
                value={format}
            ...
            <div className="space-y-2">
              <Label>{t('config.labels.audience')}</Label>
              <Select
                value={audience}
            ...
            <div className="space-y-2">
              <Label>{t('config.labels.emojis')}</Label>
              <Select
                value={emojiLevel}
        ...
        <Collapsible open={showAdvanced} onOpenChange={onShowAdvancedChange}>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" className="w-full justify-between">
              {t('config.labels.advancedOptions')}
              {showAdvanced ? (
          ...
          <CollapsibleContent className="pt-2">
            <Card>
              <CardContent className="pt-4">
                <div className="space-y-2">
                  <Label htmlFor="instructions">{t('config.labels.customInstructions')}</Label>
                  <Textarea
                    id="instructions"
                    value={customInstructions}
                    onChange={(e) => onCustomInstructionsChange(e.target.value)}
                    placeholder={t('config.placeholders.customInstructions')}
                    rows={3}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t('config.hints.customInstructions')}
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        >
          {isGenerating ? (
            <>
              ...
              {t('config.actions.generating')}
            </>
          ) : (
            <>
              <Sparkles className="mr-2 h-4 w-4" />
              {t('config.actions.generate')}
            </>
          )}
        </Button>
