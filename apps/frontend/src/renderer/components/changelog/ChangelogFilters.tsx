    ...
    <div className="w-80 shrink-0 border-r border-border overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-sm font-medium">{t('filters.labels.source')}</Label>
          <RadioGroup
            value={sourceMode}
        ...
        {sourceMode === 'git-history' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('filters.labels.gitHistoryOptions')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* History Type */}
              <div className="space-y-2">
                <Label className="text-xs">{t('filters.labels.historyType')}</Label>
                <Select
                  ...
                  <SelectContent>
                    <SelectItem value="since-version">
                      <div className="flex items-center gap-2">
                        ...
                        {t('filters.options.sinceVersion')}
                      </div>
                    </SelectItem>
                    <SelectItem value="recent">
                      <div className="flex items-center gap-2">
                        <History className="h-3 w-3" />
                        {t('filters.options.recentCommits')}
                      </div>
                    </SelectItem>
                    <SelectItem value="since-date">
                      <div className="flex items-center gap-2">
                        <Calendar className="h-3 w-3" />
                        {t('filters.options.sinceDate')}
                      </div>
                    </SelectItem>
                    <SelectItem value="tag-range">
                      <div className="flex items-center gap-2">
                        <Tag className="h-3 w-3" />
                        {t('filters.options.tagRange')}
                      </div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {gitHistoryType === 'recent' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('filters.labels.commitCount')}</Label>
                  <Input
                    type="number"
                    min={1}
                    max={500}
              ...
              {gitHistoryType === 'since-date' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('filters.options.sinceDate')}</Label>
                  <Input
                    type="date"
              ...
              {gitHistoryType === 'tag-range' && (
                <>
                  <div className="space-y-2">
                    <Label className="text-xs">{t('filters.labels.fromTag')}</Label>
                    <Select value={gitHistoryFromTag} onValueChange={onGitHistoryFromTagChange}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('filters.placeholders.selectTag')} />
                      </SelectTrigger>
                      <SelectContent>
                        {tags.map((tag) => (
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs">{t('filters.labels.toTag')}</Label>
                    <Select value={gitHistoryToTag || 'HEAD'} onValueChange={(v) => onGitHistoryToTagChange(v === 'HEAD' ? '' : v)}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('filters.placeholders.headLatest')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HEAD">{t('filters.placeholders.headLatest')}</SelectItem>
                        {tags.map((tag) => (
                          <SelectItem key={tag.name} value={tag.name}>
                            {tag.name}
                          </SelectItem>
              ...
              {gitHistoryType === 'since-version' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('filters.labels.lastVersion')}</Label>
                  <Select value={gitHistorySinceVersion} onValueChange={onGitHistorySinceVersionChange}>
                    <SelectTrigger>
                      <SelectValue placeholder={t('filters.placeholders.selectVersion')} />
                    </SelectTrigger>
                    <SelectContent>
                      {tags.map((tag) => (
                        <SelectItem key={tag.name} value={tag.name}>
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="merge-commits" className="text-xs cursor-pointer">
                  {t('filters.labels.includeMergeCommits')}
                </Label>
              </div>
              ...
              >
                {isLoadingCommits ? (
                  <>
                    ...
                    {t('filters.actions.loading')}
                  </>
                ) : (
                  <>
                    <RefreshCw className="mr-2 h-4 w-4" />
                    {t('filters.actions.loadCommits')}
                  </>
                )}
              </Button>
            </CardContent>
        ...
        {sourceMode === 'branch-diff' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('filters.labels.branchComparison')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="text-xs">{t('filters.labels.baseBranch')}</Label>
                <Select value={baseBranch} onValueChange={onBaseBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('filters.placeholders.selectBaseBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                      ...
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          ...
                          {branch.name === defaultBranch && (
                            <Badge variant="outline" className="text-xs">{t('filters.badges.default')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('filters.hints.baseBranch')}
                </p>
              </div>

              <div className="space-y-2">
                <Label className="text-xs">{t('filters.labels.compareBranch')}</Label>
                <Select value={compareBranch} onValueChange={onCompareBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('filters.placeholders.selectCompareBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                    {localBranches.map((branch) => (
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          {branch.name}
                          {branch.isCurrent && (
                            <Badge variant="secondary" className="text-xs">{t('filters.badges.current')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
