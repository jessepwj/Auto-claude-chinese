...
export function Worktrees({ projectId }: WorktreesProps) {
  const { t } = useTranslation(['workspace', 'common', 'dialogs']);
  ...
  const loadWorktrees = useCallback(async () => {
    ...
    try {
      ...
      if (taskResult.success && taskResult.data) {
        ...
        setError(taskResult.error || t('workspace:worktrees.errors.loadTaskWorktrees'));
      }
      ...
      setError(err instanceof Error ? err.message : t('workspace:worktrees.errors.loadWorktrees'));
    } finally {
      setIsLoading(false);
    }
  ...
  const handleMerge = async () => {
    ...
    if (!task) {
      setError(t('workspace:worktrees.errors.taskNotFound'));
      return;
    }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setMergeResult({
          success: false,
          message: result.error || t('workspace:worktrees.errors.mergeFailed')
        });
      }
    } catch (err) {
      setMergeResult({
        success: false,
        message: err instanceof Error ? err.message : t('workspace:worktrees.errors.mergeFailed')
      });
    } finally {
      setIsMerging(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('workspace:worktrees.errors.deleteFailed'));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t('workspace:worktrees.errors.deleteFailed'));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleDeleteTerminalWorktree = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('workspace:worktrees.errors.deleteTerminalFailed'));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t('workspace:worktrees.errors.deleteTerminalFailed'));
    } finally {
  ...
  if (!selectedProject) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-muted-foreground">{t('workspace:messages.selectProject')}</p>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
            ...
            {t('workspace:worktrees.title')}
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            {t('workspace:worktrees.subtitle')}
          </p>
        </div>
      ...
      {error && (
        <div className="mb-4 rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-sm">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-medium text-destructive">{t('workspace:worktrees.errors.error')}</p>
      ...
      {!isLoading && worktrees.length === 0 && terminalWorktrees.length === 0 && (
        <div className="flex h-full flex-col items-center justify-center text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground">{t('workspace:worktrees.emptyState.title')}</h3>
          <p className="text-sm text-muted-foreground mt-2 max-w-md">
            {t('workspace:worktrees.emptyState.description')}
          </p>
        </div>
      )}
            ...
            {worktrees.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  ...
                  {t('workspace:worktrees.sections.taskWorktrees')}
                </h3>
                      ...
                      <CardContent className="pt-0">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm mb-4">
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            ...
                            <span>{worktree.filesChanged ?? 0} {t('workspace:worktrees.stats.filesChanged')}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            <ChevronRight className="h-3.5 w-3.5" />
                            <span>{worktree.commitCount ?? 0} {t('workspace:worktrees.stats.commitsAhead')}</span>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            ...
                            {t('workspace:worktrees.actions.mergeToBase', { branch: worktree.baseBranch })}
                          </Button>
                          {task && (
                            <Button
                              variant="info"
                          ...
                          >
                            ...
                            {t('workspace:worktrees.actions.copyPath')}
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
                          ...
                          >
                            ...
                            {t('workspace:worktrees.actions.delete')}
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
            ...
            {terminalWorktrees.length > 0 && (
              <div className="space-y-4">
                <h3 className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                  ...
                  {t('workspace:worktrees.sections.terminalWorktrees')}
                </h3>
                    ...
                    <CardContent className="pt-0">
                      ...
                      {wt.createdAt && (
                        <div className="text-xs text-muted-foreground mb-4">
                          {t('workspace:worktrees.actions.created')} {new Date(wt.createdAt).toLocaleDateString()}
                        </div>
                      )}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('workspace:worktrees.merge.title')}
            </DialogTitle>
            <DialogDescription>
              {t('workspace:worktrees.merge.description')}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              <div className="rounded-lg bg-muted p-4 text-sm space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('workspace:worktrees.merge.sourceBranch')}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('workspace:worktrees.merge.targetBranch')}</span>
                  <span className="font-mono">{selectedWorktree.baseBranch}</span>
                </div>
                <div className="border-t border-border pt-3 mt-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">{t('workspace:worktrees.merge.changes')}</span>
                    <span>
                      {selectedWorktree.commitCount ?? 0} {t('workspace:worktrees.merge.commits', { filesChanged: selectedWorktree.filesChanged ?? 0 })}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className={`font-medium ${mergeResult.success ? 'text-success' : 'text-destructive'}`}>
                      {mergeResult.success ? t('workspace:worktrees.merge.success') : t('workspace:worktrees.merge.failed')}
                    </p>
                    <p className="text-muted-foreground mt-1">{mergeResult.message}</p>
                    {mergeResult.conflictFiles && mergeResult.conflictFiles.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium">{t('workspace:worktrees.merge.conflictingFiles')}</p>
              ...
              >
                {isMerging ? (
                  <>
                    ...
                    {t('workspace:worktrees.merge.merging')}
                  </>
                ) : (
                  <>
                    <GitMerge className="h-4 w-4 mr-2" />
                    {t('workspace:worktrees.merge.merge')}
                  </>
                )}
              </Button>
            )}
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('workspace:worktrees.delete.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('workspace:worktrees.delete.description')}
              {worktreeToDelete && (
                <span className="block mt-2 font-mono text-sm">
                  {worktreeToDelete.isOrphaned ? t('common:labels.orphaned') : worktreeToDelete.branch}
                </span>
              )}
              {t('workspace:worktrees.delete.warning')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {isDeleting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  {t('workspace:worktrees.delete.deleting')}
                </>
              ) : (
                <>
                  <Trash2 className="h-4 w-4 mr-2" />
                  {t('workspace:worktrees.delete.deleteButton')}
                </>
              )}
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <AlertDialog open={!!terminalWorktreeToDelete} onOpenChange={(open) => !open && setTerminalWorktreeToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('workspace:worktrees.deleteTerminal.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('workspace:worktrees.deleteTerminal.description')}
